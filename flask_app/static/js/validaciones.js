...
document.getElementById('tema').addEventListener('change', function() {
    document.getElementById('OtroContainer').classList.toggle('hidden', this.value !== 'otro');
});
...
document.getElementById('enviarBtn').addEventListener('click', function(event) {
    ...
    // Limpiar Errores 
    const Errores = document.getElementById('listErrores');
    Errores.innerHTML = ''; 
    ...
    if (fechaHoraTerminoInput.value) {
        if (!regexFechaHora.test(fechaHoraTerminoInput.value)) {
            const error = document.createElement('li');
            error.textContent = "Se debe cumplir con el siguiente formato: año-mes-día hora:minuto (ejemplo: 2025-01-18 18:00).";
            listaErrores.appendChild(error);
            errores = true;
        } else {
            ...
            if (fechaHoraTermino <= fechaHoraInicio) {
                const error = document.createElement('li');
                error.textContent = "Error en fecha de termino, debe ser luego de la hora de inicio.";
                listaErrores.appendChild(error);
                errores = true;
            }
        }
    ...
    //Validación Tema (opcional, pero si es "otro", longitud entre 3 y 15)
    ...
    //Validación foto (mínimo 1, máximo 5)
